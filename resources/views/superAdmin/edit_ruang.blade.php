...
<body>
    ...
    <main class="p-16 md:ml-64 h-auto pt-20">
        <form action="{{ route('sup-admin.ruang.update', $ruang->id) }}" method="POST">
            @csrf
            @method('PUT')
            <h2 class="text-xl font-bold mb-4">Edit Ruang</h2>
        
            <!-- Nama Ruang Input -->
            <label for="room" class="block text-sm font-medium mb-2">Nama Ruang</label>
            <input id="room" name="nama_ruang" type="text" class="w-2/4 p-2 border border-gray-400 rounded-md mb-4" value="{{ old('nama', $ruang->nama_ruang) }}">
            
            <!-- Kluster Input -->
            <label for="kluster" class="form-label font-sans font-medium m-2">Kluster:</label>
            <select name="kluster" id="kluster" class="w-fit p-2 border border-gray-400 rounded-md mb-4">
                <option  value="{{ old('nama', $ruang->kluster) }}">Select Kluster</option>
            ...
            <select name="gedung" id="gedung" class="w-fit p-2 border border-gray-400 rounded-md mb-4" required>
                <option  value="{{ old('nama', $ruang->gedung) }}">Select Gedung</option>
            </select>

            <!-- Kapasitas -->
            <label for="kapasitas" class="form-label text-sm font-medium m-2">Kapasitas</label>
            <input id="kapasitas" name="kapasitas" type="number" class="w-fit p-2 border border-gray-400 rounded-md mb-4"  value="{{ old('nama', $ruang->kapasitas) }}">
            
            <!-- Harga -->
            <label for="harga" class="form-label text-sm font-medium m-2">Harga</label>
            <input id="harga" name="harga" type="text" class="w-fit p-2 border border-gray-400 rounded-md mb-4" value="{{ old('nama', $ruang->harga) }}">
